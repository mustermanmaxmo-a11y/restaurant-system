...
import { Camera, Globe, Image as ImageIcon, Sparkles } from 'lucide-react'
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 14, background: 'var(--surface-2)', borderRadius: 8, padding: 4 }}>
        {(['screenshot', 'url'] as const).map(t => (
          <button key={t} onClick={() => { setTab(t); setResult(null); setError('') }} style={{ flex: 1, padding: '8px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: '0.82rem', fontWeight: tab === t ? 700 : 500, background: tab === t ? 'var(--accent)' : 'transparent', color: tab === t ? '#fff' : 'var(--text-muted)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
            {t === 'screenshot' ? <><Camera size={14} /> Screenshot</> : <><Globe size={14} /> URL</>}
          </button>
        ))}
      </div>
      {tab === 'screenshot' ? (
        <label style={{ display: 'block', border: '2px dashed var(--border)', borderRadius: 10, padding: '20px', textAlign: 'center', cursor: 'pointer', marginBottom: 12 }}>
          ...
          {file && previewUrl ? <img src={previewUrl} alt="" style={{ maxHeight: 140, maxWidth: '100%', borderRadius: 8 }} /> : <div style={{ color: 'var(--text-muted)', fontSize: '0.82rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px' }}><ImageIcon size={16} /> Screenshot ablegen oder klicken</div>}
        </label>
      ) : (
      ...
      {!result && (
        <button onClick={analyze} disabled={loading || (tab === 'screenshot' ? !file : !url)} style={{ display: 'inline-flex', alignItems: 'center', gap: '7px', padding: '10px 20px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff', fontWeight: 700, cursor: 'pointer', opacity: (loading || (tab === 'screenshot' ? !file : !url)) ? 0.5 : 1 }}>
          <Sparkles size={15} /> {loading ? 'Analysiert…' : 'Design erkennen'}
        </button>
      )}
