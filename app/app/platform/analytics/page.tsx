  ...
  return (
    <div style={{ padding: '32px 24px', maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ marginBottom: '28px' }}>
        <h1 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#fff', marginBottom: '6px' }}>Analytics</h1>
        <p style={{ color: '#888', fontSize: '0.85rem' }}>Alle Restaurants · Letzte 30 Tage vs. vorige 30 Tage</p>
      </div>
      ...
      <div style={{ background: '#242438', border: '1px solid #2a2a3e', borderRadius: '14px', padding: '20px', marginBottom: '20px' }}>
        <h2 style={{ color: '#fff', fontWeight: 700, fontSize: '0.9rem', marginBottom: '18px' }}>Top 10 Restaurants nach Umsatz (30d)</h2>
        {topRevenue.length === 0 ? (
          <p style={{ color: '#666', fontSize: '0.85rem' }}>Keine Bestellungen in den letzten 30 Tagen.</p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            {topRevenue.map((r, i) => (
              <Link key={r.id} href={`/platform/restaurants/${r.id}`} style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  <span style={{ color: i < 3 ? '#f59e0b' : '#555', fontSize: '0.75rem', width: '20px', textAlign: 'right', fontWeight: 700 }}>
                    {i + 1}
                  </span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                      <span style={{ color: '#ccc', fontSize: '0.83rem', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '60%' }}>{r.name}</span>
                      <div style={{ display: 'flex', gap: '12px', flexShrink: 0 }}>
                        <span style={{ color: '#555', fontSize: '0.72rem' }}>{r.orders} Orders · Ø {r.orders > 0 ? fmt(r.rev / r.orders) : '—'}</span>
                        <span style={{ color: '#fff', fontSize: '0.83rem', fontWeight: 800 }}>{fmt(r.rev)}</span>
                      </div>
                    </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px', marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '3px', height: '110px', paddingBottom: '20px' }}>
            {days14.map(day => {
              ...
              return (
                ...
                >
                  {cnt > 0 && (
                    <span style={{ color: '#555', fontSize: '0.55rem', marginBottom: '2px' }}>{cnt}</span>
                  )}
                  <div style={{
                    width: '100%', height: `${barH}%`, minHeight: cnt > 0 ? '4px' : 0,
                    background: isToday ? '#ef4444' : '#6366f1',
                    borderRadius: '2px 2px 0 0',
                  }} />
                  <span style={{
                    position: 'absolute', bottom: '-18px', fontSize: '0.55rem', color: '#444',
              ...
              return (
                ...
                >
                  <div style={{
                    ...
                    background: isBusiest ? '#ef4444' : `rgba(99,102,241,${0.15 + pct / 100 * 0.85})`,
                    borderRadius: '2px 2px 0 0',
                  }} />
                  {h % 6 === 0 && <span style={{ fontSize: '0.55rem', color: '#444', marginTop: '3px' }}>{h}h</span>}
                </div>
              )
            })}
          </div>
            ...
            {weekdays.map((day, i) => {
              ...
              return (
                <div key={day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', height: '100%', gap: '4px' }}>
                  {cnt > 0 && <span style={{ color: '#555', fontSize: '0.6rem' }}>{cnt}</span>}
                  <div style={{
                    width: '100%', height: `${Math.max(4, pct)}%`,
                    background: isBest ? '#f59e0b' : '#6366f1',
                    borderRadius: '4px 4px 0 0',
                  }} />
                  <span style={{ fontSize: '0.65rem', color: isBest ? '#f59e0b' : '#555', fontWeight: isBest ? 700 : 400 }}>{day}</span>
                </div>
              )
            })}
          </div>
        ...
        <ChartCard title="Bestellstatus-Verteilung (30d)">
          {Object.keys(statusMap).length === 0 ? (
            <p style={{ color: '#666', fontSize: '0.85rem' }}>Keine Daten.</p>
          ) : (
                ...
                return (
                  <div key={status}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px', fontSize: '0.78rem' }}>
                      ...
                      <span style={{ color: '#666' }}>{cnt} · {pct.toFixed(1)}%</span>
                    </div>
                    <div style={{ height: '5px', background: '#1a1a2e', borderRadius: '3px', overflow: 'hidden' }}>
      ...
      {upgradeCandidates.length > 0 && (
        <div style={{ background: '#242438', border: '1px solid rgba(99,102,241,0.35)', borderRadius: '14px', padding: '20px', marginBottom: '20px' }}>
          <h2 style={{ color: '#fff', fontWeight: 700, fontSize: '0.9rem', marginBottom: '6px' }}>
            🚀 Upgrade-Kandidaten — Trials mit starker Aktivität
          </h2>
          <p style={{ color: '#888', fontSize: '0.75rem', marginBottom: '16px' }}>Trial-Restaurants mit ≥5 Bestellungen in 30 Tagen — bereit für Konvertierung</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {upgradeCandidates.map(u => (
              <Link key={u.id} href={`/platform/restaurants/${u.id}`} style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(99,102,241,0.07)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '8px', cursor: 'pointer' }}>
                  <span style={{ color: '#e0e7ff', fontWeight: 600, fontSize: '0.85rem' }}>{u.name}</span>
                  <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                    <span style={{ color: '#6366f1', fontSize: '0.78rem', fontWeight: 700 }}>{u.orders} Bestellungen</span>
                    <span style={{ color: '#10b981', fontSize: '0.78rem', fontWeight: 700 }}>{fmt(u.rev)} Umsatz</span>
                    <span style={{ color: '#6366f1', fontSize: '0.72rem' }}>→ Plan setzen</span>
                  </div>
                </div>
              </Link>
            ))}
      ...
      <div style={{ background: '#242438', border: `1px solid ${churnRisk.length > 0 ? 'rgba(239,68,68,0.3)' : '#2a2a3e'}`, borderRadius: '14px', padding: '20px' }}>
        <h2 style={{ color: '#fff', fontWeight: 700, fontSize: '0.9rem', marginBottom: '6px' }}>
          ⚠️ Churn-Risiko ({churnRisk.length})
        </h2>
        <p style={{ color: '#888', fontSize: '0.75rem', marginBottom: '16px' }}>Aktive bezahlte Abos ohne Bestellungen in den letzten 14 Tagen</p>
        {churnRisk.length === 0 ? (
          <p style={{ color: '#10b981', fontSize: '0.85rem' }}>✓ Kein Churn-Risiko erkannt.</p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {churnRisk.map(r => (
              <Link key={r.id} href={`/platform/restaurants/${r.id}`} style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'rgba(239,68,68,0.06)', border: '1px solid rgba(239,68,68,0.18)', borderRadius: '8px', cursor: 'pointer' }}>
                  <div>
                    <span style={{ color: '#fff', fontWeight: 600, fontSize: '0.85rem' }}>{r.name}</span>
                    <span style={{ color: '#555', fontSize: '0.72rem', marginLeft: '8px' }}>/{r.slug}</span>
                  </div>
                  <span style={{ color: '#fca5a5', fontSize: '0.75rem', fontWeight: 700 }}>{r.plan} · anklicken →</span>
                </div>
              </Link>
            ))}
          </div>
...
function KPI({ label, value, growth, diff, warn, sub }: {
  ...
  return (
    <div style={{ background: '#242438', border: `1px solid ${warn ? 'rgba(239,68,68,0.4)' : '#2a2a3e'}`, borderRadius: '14px', padding: '16px 18px' }}>
      <p style={{ color: '#666', fontSize: '0.65rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>{label}</p>
      <p style={{ color: warn ? '#ef4444' : '#fff', fontWeight: 800, fontSize: '1.45rem', lineHeight: 1, marginBottom: '4px' }}>{value}</p>
      {sub && <p style={{ color: '#555', fontSize: '0.68rem' }}>{sub}</p>}
      {hasGrowth && (
        <span style={{ fontSize: '0.7rem', fontWeight: 700, color: (growth ?? 0) >= 0 ? '#10b981' : '#ef4444' }}>
          {(growth ?? 0) >= 0 ? '+' : ''}{growth!.toFixed(1)}% vs. -30d
        </span>
      )}
      {hasDiff && (
        <span style={{ fontSize: '0.7rem', fontWeight: 700, color: diff! > 0 ? '#10b981' : '#ef4444' }}>
          {diff! > 0 ? '+' : ''}{diff} vs. -30d
        </span>
      )}
    </div>
...
function ChartCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background: '#242438', border: '1px solid #2a2a3e', borderRadius: '14px', padding: '20px' }}>
      <h2 style={{ color: '#fff', fontWeight: 700, fontSize: '0.82rem', marginBottom: '16px' }}>{title}</h2>
      {children}
    </div>
  )
}
